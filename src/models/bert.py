...
class flaubert(nn.Module):  # equal to "BertClassifier"
    def __init__(self, output_dim, if_output_attentions, dropout=0.1):
        ...
        self.bert_config = FlaubertConfig.from_pretrained(
            args["model"], 
            output_attentions = if_output_attentions)   
        ...
        #     ),
        #     )
        self.wrapper = FlaubertModel.from_pretrained(args["model"], config=self.bert_config)

        self.dropout = nn.Dropout(p = self.dropout)
        self.output_dim = output_dim

        self.output_layer = nn.Linear(self.bert_config.hidden_size, output_dim) #self.wrapper.model.config.hidden_size
        torch.nn.init.xavier_uniform_(self.output_layer.weight)
        self.output_layer.bias.data.fill_(0.0)
    
    # def generate(self, input_ids):

    #     outputs = self.wrapper.generate(input_ids)
    #     return outputs
    ...
    def forward(self, **inputs):
        ...
        self.weights_or = attentions[-1].clone().detach().requires_grad_(True)
...
from transformers import MT5ForConditionalGeneration, MT5Model
class mt5(nn.Module):  # equal to "BertClassifier"
    def __init__(self, output_dim, if_output_attentions, dropout=0.1):
        ...
        #     ),
        #     )
        self.wrapper = MT5ForConditionalGeneration.from_pretrained(args["model"], config=self.bert_config)
    ...
    def forward(self, **inputs):
        ...
        output = self.wrapper(
                input_ids = inputs["input_ids"].to(device), 
                attention_mask = inputs["attention_mask"].to(device),
                labels = inputs["decoder_input_ids"].to(device),
                #decoder_attention_mask = inputs["decoder_attention_mask"].to(device),
                #ig = inputs["ig"],
                return_dict = True,
                output_attentions = True,
        ...
        #print(f"==>> logits.shape: {logits.shape}") # torch.Size([8, 44, 250112])
        
        past_key_values=output.past_key_values
        decoder_hidden_states=output.decoder_hidden_states
        decoder_attentions=output.decoder_attentions
        cross_attentions=output.cross_attentions
        # print(f"==>> type(cross_attentions): {type(cross_attentions)}, and length {len(cross_attentions)}") # length = 8 multi-head
        # print(f"==>> cross_attentions[0].shape: {cross_attentions[0].shape}") # torch.Size([8, 6, 44, 44])
        # print(f"==>> cross_attentions[0]: {cross_attentions[0]}")
        encoder_last_hidden_state=output.encoder_last_hidden_state
        encoder_hidden_states=output.encoder_hidden_states
        encoder_attentions=output.encoder_attentions  # tuple 1 tuple 8 

        # print("1".center(50, "-"))
        # print(len(decoder_attentions), len(encoder_attentions), len(cross_attentions)) # 8 8 8
        # print("2".center(50, "-"))
        # print(len(decoder_attentions[0]), len(encoder_attentions[0])) # 8 8 
        # print("3".center(50, "-"))
        # print(len(decoder_attentions[0][0]), len(encoder_attentions[0][0])) # 6 6
        # print("4".center(50, "-"))
        # print(len(decoder_attentions[0][0].size()), len(encoder_attentions[0][0].size())) # 3 3
        # print(f"==>> type(attention_weights): {type(encoder_attentions)}")
        ...
        #     if i ==0: selected_logits = logits[:, 0, int(torch.unique(num_index_dict[i][0]).item())].unsqueeze(1)
        #     else: 
        #         #print(' //////////// ', int(torch.unique(num_index_dict[i][0]).item()))
        #         selected_logit = logits[:, 0, int(torch.unique(num_index_dict[i][0]).item())].unsqueeze(1)
        #         #print(f"==>> selected_logit: {selected_logit}")
        #         selected_logits = torch.cat((selected_logits, selected_logit), 1)
        ...
        # self.weights_or = attention_weights[-1].clone().detach().requires_grad_(True)
        # self.weights_or = torch.tensor(attention_weights.clone().detach()[-1], requires_grad=True)  # debug by cass
        self.weights_or = cross_attentions[-1].requires_grad_(True)

        if inputs["retain_gradient"]:
            # self.wrapper.model.embeddings.word_embeddings.weight.retain_grad() # different ---> softcomp: self.wrapper.word_embeds.retain_grad()
            self.wrapper.shared.weight.retain_grad()  # ---> replace the embeddings
            #self.wrapper.word_embeds.retain_grad()
...
class train_and_save_flaubert(nn.Module):  # equal to "BertClassifier"
    def __init__(self, output_dim, if_output_attentions, dropout=0.1):
