...
feat_name_dict = {"attention", "scaled attention", "gradients", "ig", "deeplift", "random", "lime"} #"gradientshap"
...
## retrieve original prediction probability
    ...
    print("".center(50, "-"))
    print("".center(50, "-"))
    print(' original prediction saved at ----> ', fname2)
    print(' model random seed ---->', model_random_seed)
...
def conduct_experiments_zeroout_(model, data, model_random_seed, use_topk, normalise): # 0 for no normalization, 1 for softmax
    ...
    fname = os.path.join(os.getcwd(),args["extracted_rationale_dir"], "importance_scores","" )
    os.makedirs(fname, exist_ok = True)
    fname = f"{fname}test_importance_scores_{model_random_seed}.npy"
    ...
    for batch in data:
        ...
        print('feat_name_dict  ===>', feat_name_dict)
    ...
    # model_abb = args["extracted_rationale_dir"].split('_')[0].split('/')[-1]
    print("".center(50, "-"))
    print("".center(50, "-"))
    print(args["model_abbreviation"])

    detailed_fname_folder = args["evaluation_dir"] + args["model_abbreviation"]
    detailed_fname = f"{detailed_fname_folder}/ZEROOUT-faithfulness-scores-normal_{normalise}.npy"
        #description_fname = args["evaluation_dir"] + f"ATTENTION-faithfulness-scores-description.json"
    os.makedirs(detailed_fname_folder, exist_ok=True)
    ...
    description_fname = detailed_fname_folder + f"/ZEROOUT-faithfulness-scores-normal_{normalise}.json"
    #np.save(detailed_fname, faithfulness_results)
