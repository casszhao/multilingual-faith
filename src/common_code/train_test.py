    ...
    for epoch in range(epochs):
        ...
        for batch in training:
            ...
            if args.inherently_faithful:
              ...
              # for spanish_csl french_csl, need this it to work, but for chinese csl it works
                loss = loss_function(yhat, batch["labels"])#.to(device)
...
def test_model_flaubert(model, loss_function, data, save_output_probs = True, random_seed = None, for_rationale = False, variable = False):
...
def train_model_flaubert(model, training, development, loss_function, optimiser, seed,
            run, epochs = 10, cutoff_len = 2, cutoff = True, save_folder  = None, 
            ):
...
def test_model_mt5(model, loss_function, data, save_output_probs = True, random_seed = None, for_rationale = False, variable = False):
    ...
    with torch.no_grad():
        ...
        for batch in data:
            ...
            print("batch['input_ids']".center(50, "-"))
            print(batch['input_ids'])
            outputs =  model.generate(batch['input_ids'])
            print(f"==>> outputs: {outputs}")
            print("".center(50, "-"))
            print("tokenizer output".center(50, "-"))
