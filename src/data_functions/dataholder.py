...
from transformers import MT5Tokenizer, MT5TokenizerFast, MT5ForConditionalGeneration
import json
import re
...
class BERT_HOLDER():
    ...
    def __init__(self, path = str, b_size = 8 , #mask_list = list, 
        ...
        if 'csl' in args.dataset:
            ...
            max_len = 128
        elif 'chinese_news' in args.dataset:
            max_len = 128
        else: pass
        ...
        if args.query:
            if 'paws' in args['dataset'] or args['dataset'] == 'arabic_xnli':
...
class mT5_HOLDER():
    ...
    def __init__(self, path = str, b_size = 8 , #mask_list = list, 
        ...
        ## if we are dealing with a query we need to account for the query length as well
        print('using mT5 dataholder. One example from train ',  train[:1])

        if args.query:
            print('including query ')
            max_len = max([len(str(x["document"]).split()) for x in train]) + \
                            max([len(str(x["query"]).split()) for x in train])
            max_len = round(max_len)
        else:
            max_len = round(max([len(x["text"].split()) for x in train]))
        ...
        self.tokenizer = MT5TokenizerFast.from_pretrained(pretrained_weights, local_files_only=False) # by cass ood time dataholders.py (, local_files_only=True)
        ...
        if args.query:
            train = [encode_plusplus_t5(dic, self.tokenizer, max_len, label_input_indexd_dict, False, dic["document"], dic["query"]) for dic in train]
            dev = [encode_plusplus_t5(dic, self.tokenizer, max_len, label_input_indexd_dict, True, dic["document"], dic["query"]) for dic in dev]
            test = [encode_plusplus_t5(dic, self.tokenizer, max_len, label_input_indexd_dict, True, dic["document"], dic["query"]) for dic in test]

        else:
            train = [encode_plusplus_t5(dic, self.tokenizer, max_len, label_input_indexd_dict, False, dic["text"]) for dic in train]
            dev = [encode_plusplus_t5(dic, self.tokenizer, max_len, label_input_indexd_dict, True, dic["text"]) for dic in dev]
            test= [encode_plusplus_t5(dic, self.tokenizer, max_len, label_input_indexd_dict, True, dic["text"]) for dic in test]
