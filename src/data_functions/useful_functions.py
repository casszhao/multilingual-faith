...
print('args in useful functions ', args)
...
def contigious_(importance_scores, tokens_to_mask):
    ...
    ngram = torch.stack([importance_scores[i:i + tokens_to_mask]
                        for i in range(len(importance_scores) - tokens_to_mask + 1)])
    indxs = [torch.arange(i, i+tokens_to_mask)
             for i in range(len(importance_scores) - tokens_to_mask + 1)]
...
def mask_contigious(sentences, scores, length_to_mask):
    ...
    if len(sentences.size()) < 2:  # single instance
        ...
        ngram = torch.stack([scores[k:k + length_to_mask]
                            for k in range(len(scores) - length_to_mask + 1)])
        indxs = [torch.arange(j, j+length_to_mask)
                 for j in range(len(scores) - length_to_mask + 1)]
        ...
        for i in range(scores.size(0)):
            ...
            ngram = torch.stack([score[k:k + length_to_mask]
                                for k in range(len(score) - length_to_mask + 1)])
            indxs = [torch.arange(j, j+length_to_mask)
                     for j in range(len(score) - length_to_mask + 1)]
    ...
    for _i_ in range(importance_scores.size(0)):
        ...
        if batch_input_ids is not None:
            ...
            if args['model_abbreviation'] == 't5m':
                sos_eos = torch.where(
                    batch_input_ids[_i_] == special_tokens["eos_token_id"][0].item())[0]
            else:
                sos_eos = torch.where(
                    batch_input_ids[_i_] == special_tokens["sep_token_id"][0].item())[0]
...
def create_only_query_mask_(batch_input_ids: torch.tensor, special_tokens: dict):
    ...
    for seq in batch_input_ids:
        ...
        if args['model_abbreviation'] == 't5m':
            sos_eos = torch.where(
                seq == special_tokens["eos_token_id"][0].item())[0]
        else:
            sos_eos = torch.where(
                seq == special_tokens["sep_token_id"][0].item())[0]
...
def describe_data_stats(path_to_data, path_to_stats):
    ...
    for split_name in descriptions.keys():
        ...
        data = pd.read_csv(
            f"{path_to_data}{split_name}.csv").to_dict("records")

        if "query" in data[0].keys():

            avg_ctx_len = np.asarray(
                [len(str(x["document"]).split(" ")) for x in data]).mean()
            avg_query_len = np.asarray(
                [len(str(x["query"]).split(" ")) for x in data]).mean()
            ...
            avg_seq_len = np.asarray(
                [len(x["text"].split(" ")) for x in data]).mean()

            descriptions[split_name]["avg. sequence length"] = int(avg_seq_len)

        descriptions[split_name]["no. of documents"] = int(len(data))

        label_nos = np.unique(np.asarray(
            [x["label"] for x in data]), return_counts=True)

        for label, no_of_docs in zip(label_nos[0], label_nos[1]):

            descriptions[split_name][f"docs in label-{label}"] = int(
                no_of_docs)
...
def encode_plusplus_(data_dict, tokenizer, max_length, *arguments):
    ...
    if len(arguments) > 1:
        ...
        if 'roberta-hindi' in str(tokenizer): init_mask_ = model_inputs["input_ids"].size(-1)
        else:
            init_mask_ = torch.where(
            model_inputs["input_ids"] == tokenizer.sep_token_id)[1][0]
      ...
      # preserve cls token
        ...
        if tokenizer.name_or_path == "flax-community/roberta-hindi":
            init_mask_ = model_inputs["attention_mask"].sum()-1
            #print('using hindi_roberta')
        else:
...
def encode_plusplus_t5(data_dict, tokenizer, max_length, label_input_indexd_dict, if_dev, *arguments):
    """
    returns token type ids, padded doc and 
    """
    ...
    if len(arguments) > 1:
        ...
        # i.e. keeping only the contxt not the query
        init_mask_ = torch.where(
            model_inputs["input_ids"] == tokenizer.eos_token_id)[1][0]
        fin_mask = model_inputs["input_ids"].size(-1)
        range_to_zero = torch.arange(init_mask_, fin_mask)
        model_inputs["query_mask"] = model_inputs["attention_mask"].clone()
        model_inputs["query_mask"].squeeze(0)[range_to_zero] = 0
    ...
    if if_dev:
        ...
        model_inputs["decoder_input_ids"] = torch.tensor([0, 1]).to(device)
        model_inputs["decoder_attention_mask"] = torch.tensor(
            [1, 1]).to(device)
...
def encode_plusplus_t5_backup(data_dict, tokenizer, max_length, *arguments):
    """
    returns token type ids, padded doc and 
    """
